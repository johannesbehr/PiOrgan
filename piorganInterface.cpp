...
void staticOnTick(int ticks, int bpm){
    instance->onTick(ticks, bpm);
}

void PiOrganInterface::setResetScreenSaver(void (*fnk)()){
    myPiOrgan.setMusicKeyPressed(fnk);
}

void PiOrganInterface::onTick(int ticks, int bpm){
    // Berechne die Gesamtzeit in Sekunden
    double totalTime = (ticks * 60.0) / (bpm * 480.0);
    // Extrahiere Minuten und Sekunden
    int min = static_cast<int>(totalTime) / 60;
    int sec  = static_cast<int>(totalTime) % 60;

    if(last_min!=min || last_sec != sec){
        last_min = min;
        last_sec = sec;
        ...
        emit signalSetProperty(txtMidiTime, "text", QString::fromUtf8(buffer));
    }
...
PiOrganInterface::PiOrganInterface(QObject *parent) : QObject(parent)
{
    ...
    myPiOrgan.setTickHandler(staticOnTick);

    connect(this, SIGNAL(signalSetProperty(QObject*, const char*, const QVariant&)), this, SLOT(setProperty(QObject*, const char*, const QVariant&)));
}

void PiOrganInterface::setProperty(QObject *obj, const char *name, const QVariant &value){
...
void PiOrganInterface::setRootObject(QObject *rootObject){
    ...
    pltBtnDisabled = rootObject->findChild<QObject*>("pltBtnDisabled");
    ...
    lvMidiFiles = rootObject->findChild<QObject*>("lvMidiFiles");
    ...
    btnLoadSoundFont = rootObject->findChild<QObject*>("btnLoadSoundFont");

    txtCurrentSoundFont = rootObject->findChild<QObject*>("txtCurrentSoundFont");

    busyIndicator = rootObject->findChild<QObject*>("busyIndicator");

    txtMidiTime = rootObject->findChild<QObject*>("txtMidiTime");
    btnStopMidiFile = rootObject->findChild<QObject*>("btnStopMidiFile");
...
void PiOrganInterface::loadMidiFile(const QString &midiFile){
    ...
    //     t.detach();
    // }
    myPiOrgan.loadMidiFile(midiFile.toStdString().c_str());
    btnStopMidiFile->setProperty("palette",  QVariant::fromValue(pltBtnRegular));
    btnStopMidiFile->setProperty("enabled",  "true");
    btnPlayMidiFile->setProperty("palette",  QVariant::fromValue(pltBtnDisabled));
    btnPlayMidiFile->setProperty("enabled",  "false");     
}
void PiOrganInterface::playMidiFile(){
    // if(selectedSoundFont!=""){
    //     qDebug() << "Load SoundFont:" << selectedSoundFont.c_str();
    //     btnLoadSoundFont->setProperty("enabled",  "false");
    //     btnLoadSoundFont->setProperty("palette",  QVariant::fromValue(pltBtnDisabled));
    ...
    myPiOrgan.playMidiFile();
    btnStopMidiFile->setProperty("palette",  QVariant::fromValue(pltBtnRegular));
    btnStopMidiFile->setProperty("enabled",  "true");
    btnPlayMidiFile->setProperty("palette",  QVariant::fromValue(pltBtnDisabled));
    btnPlayMidiFile->setProperty("enabled",  "false");
}
void PiOrganInterface::stopMidiFile(){
    // if(selectedSoundFont!=""){
    //     qDebug() << "Load SoundFont:" << selectedSoundFont.c_str();
    //     btnLoadSoundFont->setProperty("enabled",  "false");
    //     btnLoadSoundFont->setProperty("palette",  QVariant::fromValue(pltBtnDisabled));
    ...
    //     t.detach();
    // }
    printf("Stop Midi file...\n");
    myPiOrgan.stopMidiFile();
    btnPlayMidiFile->setProperty("palette",  QVariant::fromValue(pltBtnRegular));
    btnPlayMidiFile->setProperty("enabled",  "true");
    btnStopMidiFile->setProperty("palette",  QVariant::fromValue(pltBtnDisabled));
    btnStopMidiFile->setProperty("enabled",  "false");
}

void PiOrganInterface::midiSpeedValueChanged(const double value){
    double s = 0.5 + (value * 1.5);
    myPiOrgan.setMidiFileTempo(s);
}
...
void PiOrganInterface::loadSoundFontThread(){
    printf("Load thread started...\n");
    myPiOrgan.loadSoundFont(selectedSoundFont.c_str());
    onSoundFontLoaded(selectedSoundFont);
    printf("Load thread done.\n");
}


void PiOrganInterface::onSoundFontLoaded(std::string soundFont){
    // Setzte Informationen über Soundfont auf Maske
    // txtCurrentSoundFont->setProperty("text", QString::fromStdString(soundFont));
    // btnLoadSoundFont->setProperty("enabled",  "true");
    // btnLoadSoundFont->setProperty("palette",  QVariant::fromValue(pltBtnRegular));
    // busyIndicator->setProperty("running",  "false");
    emit this->signalSetProperty(txtCurrentSoundFont, "text", QString::fromStdString(soundFont));
    emit this->signalSetProperty(btnLoadSoundFont, "enabled",  "true");
    emit this->signalSetProperty(btnLoadSoundFont, "palette",  QVariant::fromValue(pltBtnRegular));
    emit this->signalSetProperty(busyIndicator, "running",  "false");
...
QStringList  PiOrganInterface::getMidiFiles(){
  QStringList model;
 
    std::vector<std::string> midiFiles = myPiOrgan.getMidiFiles();

    for (const auto& file : midiFiles) {
        model.append(QString(file.c_str()));    
    }
    return model;
}
