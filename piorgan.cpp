...
PiOrgan* piorgan_instance = NULL;

int static_handle_tick(void *data, int tick)
{
	...
	piorgan_instance->onTick(data, tick);
	return FLUID_OK;
}
		...
		switch (ev->type)
		{
			...
			case SND_SEQ_EVENT_NOTEON:
			{
				...
				if (vel > 0)
					...
					fluid_synth_noteoff(synth, channel, note); // NoteOn vel=0 = NoteOff
				break;
			}
			...
			{
				int value = ev->data.control.value; // -8192 .. +8191
			...
			{
				int note = ev->data.note.note;
				int val  = ev->data.note.velocity;

				fluid_synth_key_pressure(synth, channel, note, val);
				break;
			}

			// ---------------- BANK SELECT (MSB/LSB via CC) ----------------
			// läuft eigentlich schon über CONTROLLER (CC0 / CC32)
			// optional: nichts extra nötig

			// ---------------- SYSTEM / IGNORE ----------------
			default:
				// Optional: Debug
				// printf("Unhandled MIDI event: %d\n", ev->type);
				break;
		}
	}
...
void PiOrgan::setTickHandler(void (*fnk)(int, int)){
	tick_ = fnk;
}
...
void PiOrgan::loadSoundFont(const char *soundfont){
	...
	void* d = fluid_sfont_get_data(sfont);
	
	const char* sname = fluid_sfont_get_name(sfont);	
...
void PiOrgan::playMidiFile(){
	if(player!=NULL){
		fluid_player_play(player);
	}
}

void PiOrgan::stopMidiFile(){
	fluid_player_stop(player);
}

void PiOrgan::setMidiFileTempo(double tempo){
	midiFileTempo = tempo;
	fluid_player_set_tempo(player, FLUID_PLAYER_TEMPO_INTERNAL , midiFileTempo);
}

void PiOrgan::loadMidiFile(const char* file){
	char midifile[100];
	printf("Load %s\n", file);

	sprintf(midifile, "/home/pi/Musik/%s", file);
	...
	fluid_player_set_tick_callback(player, static_handle_tick, player);
	bpm = fluid_player_get_bpm(player);
...
std::vector<std::string> PiOrgan::getMidiFiles() {
    std::vector<std::string> midiFiles;
	string directoryPath= "/home/pi/Musik";

    for (const auto& entry : std::filesystem::directory_iterator(directoryPath)) {
        if (entry.is_regular_file() && entry.path().extension() == ".mid") {
            midiFiles.push_back(entry.path().filename().string());
        }
    }
